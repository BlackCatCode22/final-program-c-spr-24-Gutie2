...
public:
	...
	const string& get PhoneNumber() const {
		return phoneNumber;
	}
...
int linearSearch(const vector<Contact>& contacts, const string& target) {
	...
	cout << "Linear search iteration: " << iterations << endl;
	return -1;
}
...
int main() {
	...
	if (resultLinear != -1) {
		cout << "Contact found by linear search at index: " << resultLinear << end1;
	} else {
		cout << "Contact not found by linear search." << end1;
	}
	
	// Perform Binary Search
	int resultBinary = binarySearch(contacts, targetName);
	if (resultBinary != -1) {
		cout << "Contact found by binary search at index: " << resultBinary << end1;
	} else {
		cout << "Contact not found by binary search." << end1;
	}
